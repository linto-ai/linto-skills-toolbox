        ...
        try {
          for (let keyIntent in json.intent) {
            for (let langue in json.intent[keyIntent]) {
              ...
              await this.makeRequest('POST', lmData.url + '/model/' + modelName + '/intent/' + keyIntent, path, true)
          ...
          for (let keyEntities in json.entities) {
            for (let langue in json.entities[keyEntities]) {
              ...
              let path = tempFs.writeFileSync(data)
              await this.makeRequest('POST', lmData.url + '/model/' + modelName + '/entity/' + keyEntities, path, true)
              if (modelArr.indexOf(modelName) === -1)
                modelArr.push(modelName)
            }
          }
      ...
      .then(async () => {
        try {
          for (let i in modelArr) {
            await this.makeRequest('POST', lmData.url + '/model/' + modelArr[i], modelArr[i].split('_')[1], false)
          }
        } catch (e) {
          debug('err', e)
        }
